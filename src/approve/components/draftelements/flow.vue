<template>
    <div class="flow-wrapper">
        ...
        <div class="flow-item" v-for="userTask in assigneeInfo">
            ...
            <div class="detail-wrapper">
                <div class="detail-header"  style="-webkit-box-orient:vertical">
                    ...
                    <div class="edit-wrapper">
                        <span class="edit"
                              @click="addOrEditItem(userTask.taskId, userTask.isAssigneeChoice)"
                              :class="{able: userTask.isAssigneeChoice && userTask.value,
                               add: userTask.isAssigneeChoice && !userTask.value}">
                        </span>
                        <span class="delete" :class="{able: userTask.isPromoterDelete}"
                              @click="doDeleteItem(userTask.taskId, userTask.isPromoterDelete, userTask.name)"></span>
                    </div>
                </div>
                <div class="detail">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            doDeleteItem (taskId, ifDelete, userTaskName) { // 删除
                if (ifDelete) {

                    MessageBox.confirm('确认删除该节点？').then(action => {
                        for (let i = 0; i < this.assigneeInfo.length; i++) {
                            if (this.assigneeInfo[i].taskId == taskId) {
                                this.assigneeInfo.splice(i, 1); // 删除
                            }
                        }

                        this.SAVE_ASSIGNEE(this.assigneeInfo); // 保存
                        // 删完后
                        if (this.assigneeInfo.length <=0) {
            ...
            async addOrEditItem (taskId, ifAddOrEdit) {
                // 联调 调取通讯录
                // 根据id 确定哪一条， 替换数据
                if (ifAddOrEdit) {
                    // 触发控件
                    ...
                    if (staffData.staffId) {
                        ...
                        for (let i = 0; i < this.assigneeInfo.length; i++) {
                            if (this.assigneeInfo[i].taskId == taskId) {
                                // 添加或者替换
                                this.assigneeInfo[i].value = staffData.staffId;
                                this.assigneeInfo[i].staffInfoToUser = [];
                                if (this.staffsList) {

                                    for (let j = 0; j < this.staffsList.length; j++) {
                                        let staffItem = this.staffsList[j];
                                        if (Number(staffData.staffId) === Number(staffItem.id)) {
                                            this.assigneeInfo[i].staffName = staffItem.staffName;
                                            if (staffItem.partTime === 0) { // 主职位
                                                this.assigneeInfo[i].staffInfoToUser.unshift(staffItem);
                                            } else { // 兼职
                                                this.assigneeInfo[i].staffInfoToUser.push(staffItem);
                                            }
                                        }
                                    }
                                }
        ...
        created () {
            // 不能修改时保存的数据
            this.SAVE_ASSIGNEE(this.assigneeInfo);
            // 默认流程控制保存为true
            this.$store.state.saveFlow = true;
            this.$store.state.flowStatus = true;
